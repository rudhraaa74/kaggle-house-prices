...
# --- START PHASE 5 MITIGATION ---
print("Phase 5: Constrained Optuna + Robust Stacking", flush=True)
...
print("Tuning Regularized XGBoost...", flush=True)
def objective_xgb(trial):
    params = {
        'n_estimators': trial.suggest_int('n_estimators', 500, 2000),
        'learning_rate': trial.suggest_float('learning_rate', 0.01, 0.1, log=True),
        'max_depth': trial.suggest_int('max_depth', 2, 4), # Constrained
        'min_child_weight': trial.suggest_int('min_child_weight', 5, 20), # Constrained
        'subsample': trial.suggest_float('subsample', 0.4, 0.8), # Constrained
        'colsample_bytree': trial.suggest_float('colsample_bytree', 0.4, 0.8), # Constrained
        'reg_alpha': trial.suggest_float('reg_alpha', 1e-4, 1.0, log=True),
        'reg_lambda': trial.suggest_float('reg_lambda', 1.0, 100.0, log=True), # Constrained heavily
        'random_state': 42,
        'n_jobs': 1
    }
    model = create_pipeline(XGBRegressor(**params))
    return cv_rmse_r2(model, X_train_inter, y_train)

study_xgb = optuna.create_study(direction='minimize')
study_xgb.optimize(objective_xgb, n_trials=30)
best_xgb_params = study_xgb.best_params
print(f"| Regularized XGBoost CV RMSE | {study_xgb.best_value:.4f} |", flush=True)

print("Tuning Regularized LightGBM...", flush=True)
def objective_lgbm(trial):
    params = {
        'n_estimators': trial.suggest_int('n_estimators', 500, 2000),
        'learning_rate': trial.suggest_float('learning_rate', 0.01, 0.1, log=True),
        'max_depth': trial.suggest_int('max_depth', 2, 4), # Constrained
        'min_child_samples': trial.suggest_int('min_child_samples', 15, 60), # Constrained
        'subsample': trial.suggest_float('subsample', 0.4, 0.8), # Constrained
        'colsample_bytree': trial.suggest_float('colsample_bytree', 0.4, 0.8), # Constrained
        'reg_alpha': trial.suggest_float('reg_alpha', 1e-4, 1.0, log=True),
        'reg_lambda': trial.suggest_float('reg_lambda', 1.0, 100.0, log=True), # Constrained heavily
...
print(f"| Regularized LightGBM CV RMSE | {study_lgbm.best_value:.4f} |", flush=True)

print("Stacking with RidgeCV...", flush=True)
...
stacked_score_r2 = np.sqrt(-cross_val_score(stacked_model_r2, X_train_inter, y_train, scoring="neg_mean_squared_error", cv=kf, n_jobs=1)).mean()
print(f"| Regularized StackingRegressor (RidgeCV) CV RMSE | {stacked_score_r2:.4f} |", flush=True)
...
code_final = f"""# Phase 5: Mitigation Strategy (Robust Regularization)
...
print("Mitigation script complete. Notebook updated!")
