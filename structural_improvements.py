...
def evaluate_pipeline(apply_improvements=False):
    # --- LOAD DATA ---
    ...
    if apply_improvements:
        train = train[train['SaleCondition'] == 'Normal'].copy()
    ...
    if apply_improvements:
        steps.append(('knn_imputer', AdvancedKNNImputer(cat_cols=genuine_cat, num_cols=genuine_num)))
    steps.append(('ordinal_map', OrdinalMapper()))
    ...
    return rmse, stacked_model, X_train_inter, y_train, X_test_inter, test_id

print("Running Baseline (Phase 4, Regularized) ...", flush=True)
rmse_baseline, _, _, _, _, _ = evaluate_pipeline(apply_improvements=False)

print("Running with Structural Improvements (Normal only, KNN, Yeo-Johnson) ...", flush=True)
rmse_improved, best_model, X_train_opt, y_train_opt, X_test_opt, test_id = evaluate_pipeline(apply_improvements=True)

print("\n=== Structural Improvements Comparison ===")
print(f"Baseline Phase 4 CV RMSE: {rmse_baseline:.5f}")
print(f"Improved Phase 4 CV RMSE: {rmse_improved:.5f}")

# Train and Predict
best_model.fit(X_train_opt, y_train_opt)
final_preds = np.expm1(best_model.predict(X_test_opt))
...
cells_to_append = []
code_final = f"""# Phase 6: Structural Improvements & Generalization
